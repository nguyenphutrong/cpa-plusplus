...
func (s *Service) registerModelsForAuth(ctx context.Context, a *coreauth.Auth) {
	...
	case "antigravity":
		...
		models = applyExcludedModels(models, excluded)
	case "github-copilot":
		if live := liveInventoryModelInfos(a, provider, "github-copilot", "github-copilot"); len(live) > 0 {
			models = live
		} else {
		...
		models = applyExcludedModels(models, excluded)
	case "kiro":
		if live := liveInventoryModelInfos(a, provider, "kiro", "kiro"); len(live) > 0 {
			models = live
		} else {
...
func (s *Service) refreshModelRegistrationForAuth(current *coreauth.Auth) bool {
	...
	if refreshed := s.refreshLiveModelInventory(ctx, current); refreshed != nil && refreshed != current {
		current = refreshed
		if updated, errUpdate := s.coreManager.Update(ctx, current); errUpdate == nil && updated != nil {
			current = updated
		} else if errUpdate != nil {
	...
	if refreshed := s.refreshLiveModelInventory(ctx, latest); refreshed != nil && refreshed != latest {
		latest = refreshed
		if updated, errUpdate := s.coreManager.Update(ctx, latest); errUpdate == nil && updated != nil {
			latest = updated
		} else if errUpdate != nil {
...
func (s *Service) syncProviderModelsFromManagement(ctx context.Context, auth *coreauth.Auth) (*coreauth.Auth, []*registry.ModelInfo, error) {
	...
	s.registerModelsForAuth(ctx, next)
